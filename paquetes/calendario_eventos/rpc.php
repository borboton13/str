<?php
	...
	case 'startCalendar':
		...
		for($i=0; $i<date("t", $thisDate); $i++)
		{
			...
			$dayCheck = mysql_query("SELECT id_st_cronograma_informes FROM st_cronograma_informes WHERE fecha=FROM_UNIXTIME($finalDate + 3600,'%Y-%m-%d') LIMIT 1", $conexion);
	...
	case 'listEvents':
		...
		$eventQuery = mysql_query("SELECT st.id_st_cronograma_informes, concat(u.nombre, ' ', u.ap_pat) as id_usuario, t.ubicacion 
